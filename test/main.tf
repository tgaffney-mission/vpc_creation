...
resource "aws_subnet" "additional" {
  count = var.create_vpc && length(var.additional_subnets) > 0 ? length(var.additional_subnets) : 0

  vpc_id                          = local.vpc_id
  cidr_block                      = keys(var.additional_subnets)[count.index]
  availability_zone               = element(var.azs, count.index)
  assign_ipv6_address_on_creation = var.private_subnet_assign_ipv6_address_on_creation == null ? var.assign_ipv6_address_on_creation : var.private_subnet_assign_ipv6_address_on_creation

  ipv6_cidr_block = var.enable_ipv6 && length(var.additional_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.additional_subnet_ipv6_prefixes[count.index]) : null

  tags = merge(
    {
      "Name" = format(
        "%s-${var.additional_subnet_suffix}-%s",
        var.name,
        element(var.azs, count.index),
      )
    },
...
resource "aws_route_table_association" "additional" {
  ...
  route_table_id = join(values(var.additional_subnets)[count.index], ["aws_route_table.", "[0].id]"])
}
